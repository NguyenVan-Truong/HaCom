...
const ActionSlider = () => {
    ...
    return (
        <div className='flex flex-wrap justify-center'>
            <div className="flex items-center justify-center w-full max-w-[1500px] overflow-x-hidden">
                ...
                <div className="overflow-hidden w-[80%] max-w-[80%]">
                    <div
                        className="flex transition-transform duration-300 ease-in-out"
                        style={{ transform: `translateX(-${currentIndex * 20}%)` }}
                    >
                        {products.map((product, index) => (
                            <div key={index} className=" flex-shrink-0 w-[20%] p-2 box-border">
                                <div className="flex flex-col border-2 rounded-xl border-gray-500 space-y-2 pr-4 pb-4">
                                    <img src={product.imageUrl} alt="Sản phẩm" className="w-full h-auto rounded-lg object-contain" />
                                    <div className="flex justify-between pl-2 items-center space-x-2">
                                        <div className="flex">
                                            {[...Array(5)].map((_, i) => (
                                    ...
                                    <h3 className="text-sm font-medium pl-2 text-left">{product.name}</h3>
                                    <p className="text-base pl-2">
                                        <span className="line-through text-gray-500">{product.originalPrice}</span>
                                        <span className="text-red-600">{product.discount}</span>
                                    </p>
                                    <p className="text-xl pl-2 font-bold">{product.discountedPrice}</p>
                                    <div className="flex items-center pl-2 text-blue-600 space-x-1">
